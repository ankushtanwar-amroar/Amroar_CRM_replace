...
Contains helper functions used by multiple route modules.
Import these from server.py to avoid code duplication.
"""
...
async def evaluate_formula_fields_for_record(
    ...
    Supports parent field references like:
    - Account.Industry
    - Account.Customer_Type  
    - Parent.field_name (legacy syntax)
    """
    ...
    if parent_field_paths:
        try:
            ...
            logger.debug(f"Resolved parent fields for formula: {parent_values}")
    ...
    for formula_field in formula_fields:
        try:
            ...
            # Set engine's blank handling mode
            engine.blank_as_zero = blank_as_zero
            
            # Substitute parent field references in the expression
            substituted_expr = expression
            for parent_path, value in parent_values.items():
                # Replace Account.Industry with the actual value
            ...
            # Evaluate the formula with substituted parent values
            ...
            if error is None:
                ...
                # On error, set to None or appropriate default
                enhanced_data[api_key] = None
                logger.debug(f"Formula evaluation error for {api_key}: {error}")
                
        except Exception as e:
            # Log and continue with other formula fields
...
async def evaluate_validation_rules(
    ...
    Supports parent field references like Account.Industry, Account.Customer_Type.
    
    Returns:
        (is_valid, error_info) - (True, None) if valid, (False, error_dict) if invalid
        error_dict contains: message, error_location, error_field
    """
    try:
        ...
        logger.info(f"Found {len(rules)} active validation rules for {object_name}")
        
        # Collect all parent field paths needed for evaluation
        parent_field_paths = []
        for rule in rules:
        ...
        if parent_field_paths:
            ...
            logger.debug(f"Resolved parent fields: {resolved_parent_data}")
        ...
        for rule in rules:
            ...
            for condition in conditions:
                ...
                # Get actual value - check parent fields first, then record data
                if "." in field_name:
            ...
            # If rule condition is met, validation fails
            if rule_passes:
                error_info = {
        ...
        # On error, allow (fail-safe to prevent blocking legitimate saves)
        return True, None
...
async def log_audit_event(
    ...
    Non-blocking - failures are logged but don't stop the API.
    """
    try:
        audit_event = {
            "id": str(uuid.uuid4()),
        ...
        # Non-blocking: log error but don't raise exception
...
async def get_subordinate_user_ids(current_user: User) -> List[str]:
    ...
    Returns list of user IDs who are below current user in hierarchy.
    """
    try:
        ...
        # Find all subordinate roles (roles with parent = current role)
        subordinate_roles = []
...
async def check_permission(
    current_user: User,
    object_name: str,
    action: str  # 'create', 'read', 'edit', 'delete'
) -> bool:
    """
    Check if user has permission to perform action on object.
    
    Default behavior: ALLOW if permission set not found (backward compatibility).
    """
    from fastapi import HTTPException
    
    try:
        # If user has no role (backward compatibility), allow all
        if not current_user.role_id:
            logger.debug(f"User {current_user.email} has no role - allowing by default")
            return True
        ...
        # If no permission set found, allow by default (backward compatibility)
        if not permission_set:
            logger.debug(f"No permission set for role {current_user.role_id} - allowing by default")
            return True
        
        # Find permission for this specific object
        ...
        # If object not in permission set, allow by default
        if not object_permission:
            logger.debug(f"No permission defined for {object_name} - allowing by default")
            return True
        ...
        # On error, allow by default to prevent breaking existing functionality
        logger.warning("Permission check failed - allowing by default for safety")
        return True
